...
private class PdfAnnotationsImplementation(context: Context) : PdfAnnotationsApi {
    ...
    private fun addAnnotationsToPdfBox(
        pdfFilePath: String, drawingPaths: List<Map<String, Any>>?,
        textAnnotations: List<Map<String, Any>>?,
        flutterPdfPageWidth: Float, flutterPdfPageHeight: Float
    ): Boolean {
    ...
    private fun addTextAnnotations(
        ...
        for (textAnnotation in textAnnotations) {
            ...
            val scaledFontSize = fontSize.toFloat() * scalingY
            ...
            val descentHeight: Float = measurements.descentHeight
            ...
            if (coordAsScaledPoint.y <= pdfBoxPageHeight + annotationHeight) {
    ...
    private fun addFontToResources(
        document: PDDocument,
        fontName: String,
        res: PDResources
    ): String {
        val assetPath =
            FlutterInjector.instance().flutterLoader().getLookupKeyForAsset("fonts/$fontName")
    ...
    private fun convertCoordToPointInPageSpace(
        ...
        scalingX: Float, scalingY: Float, annotationHeight: Float, descentHeight: Float
    ): PointF {
        val x = coordinate[0].toFloat() * scalingX
        val y = (coordinate[1].toFloat() - pageHeightOffset - descentHeight) * scalingY - annotationHeight
        return PointF(x, y)
    }
    ...
    private fun addDrawingAnnotation(
        ...
        val coordsAreValidForThisPage =
            setDrawingRectangle(coordinates, width, pdfPageHeight, freehand)
    ...
    private fun createDrawingAnnotation(
        colourList: List<Double>,
        width: Float
    ): PDAnnotationMarkup {
        val colourComponents = floatArrayOf(
